...
<html>
<head>
    <title>PDO - Liittimen lisäys</title>
    ...
    <table class='table table-hover table-responsive table-bordered'>
        ...
        <td><?php echo "GPU ID" ?> </td>
            ...
            <?php
              ...
              $sql = "SELECT LiitinID ,Liitin FROM Liittimet";
              $komento = $yhteys->prepare($sql);
              $komento ->execute();
              $tulos = $komento->fetchAll();
              ?>
            <td><select class='form-control' name='LiitinID'>
            <?php foreach ($tulos as $row){ ?>
            <option value="<?php echo $row['LiitinID'];?>"> <?php echo $row['Liitin'];?> </option>
                <?php } ?>
            </select>
            </td>
         </tr>
...
// lomakkeelta lähetetään tietoa, tapahtuu tämä php-koodi
    ...
    $lisaysKomento = "INSERT INTO Naytonliittimet SET GPU_ID=$ID, LiitinID=:LiitinID";
	$sqlLause = $yhteys->prepare($lisaysKomento);
	  
	  // haetaan käyttäjän antamat tiedot ja putsataan ne laittomuuksista
      $LiitinID = htmlspecialchars(strip_tags($_POST["LiitinID"]));	
	
      $sqlLause->bindParam(':LiitinID', $LiitinID);	                                                        
...
<div class="container-fluid">
  ...
  <?php
    ...
    $sqlLause = "SELECT Naytonliittimet.GPU_ID AS GPU_ID, Naytonliittimet.LiitinID AS LiitinID, Liitin
    FROM Naytonliittimet 
    INNER JOIN Liittimet ON Naytonliittimet.LiitinID = Liittimet.LiitinID 
    WHERE GPU_ID = $ID";
    ...
    while ($rivi = $komento->fetch(PDO::FETCH_ASSOC)){
      ...
      // tehdään jokaisesta osastotaulun rivistä oma rivi tauluun
          ...
          echo "<td>{$Liitin}</td>";
          echo "<td>";
          echo "<a  href='deleteLiitin.php?GPU_ID={$GPU_ID}&LiitinID={$LiitinID}' onclick='return checkDelete(this.id)' class='btn btn-danger' id='$GPU_ID - $LiitinID $Liitin'>Poista Liitin</a>  &nbsp";
